...
public class LoggerAspect {
    ...
    @After(value="execution(public * de.atruvia.webapp.presentation.v1.PersonQueryController.*(..)) ")
    public void logAfterReturning(final JoinPoint joinPoint) {
        log.warn(String.format("############################# After: %s ######################", joinPoint.getSignature().getName()));

    }

    @AfterThrowing(value="execution(public * de.atruvia.webapp.presentation.v1.PersonQueryController.*(..))", throwing = "ex")
